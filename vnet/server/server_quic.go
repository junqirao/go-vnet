...
func (s *quicServer) Setup(ctx context.Context, cfg *TransportConfig) (err error) {
	...
	// g.Log().Infof(ctx, "use quic config %+v", quicConfig)
...
func (s *quicServer) GetDstTransportWriter(src *Session, dst *Session) (rwc io.ReadWriteCloser, err error) {
	...
	if err != nil {
		g.Log().Errorf(src.Ctx, "internal error: %s", err.Error())
		return
	}

	dstStream, err := conn.OpenStreamSync(src.Ctx)
	if err != nil {
		g.Log().Errorf(src.Ctx, "open stream error: dst=%v", dst.SessionId)
		return
	}
	g.Log().Infof(src.Ctx, "open stream success: dst=%v id=%d", dst.SessionId, dstStream.StreamID().StreamNum())
	rwc = dstStream
	return
}
