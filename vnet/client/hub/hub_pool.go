...
// NewEventPool 创建新的事件池
// minPoolSize: 预填充的最小对象数量（保证 GC 后仍可用）
// newFunc: 创建新对象的函数
func NewEventPool[T any](minPoolSize int, newFunc func() T) *EventPool[T] {
	p := &EventPool[T]{
		pool: sync.Pool{
			New: func() any {
				return newFunc()
