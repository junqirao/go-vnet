...
type (
	IQuota interface {
		SubmitUsage(ctx context.Context, id int, target string, targetType string, usage int64, startTime time.Time) (err error)
		LoadUsage(ctx context.Context, id int, target string, targetType string) (int64, error)
		GetById(ctx context.Context, id int) (quota *entity.Quota, err error)
		...
		GetAdaptor(ctx context.Context, id int, target string, targetType string) (a quota.Adaptor, err error)
	}
)
